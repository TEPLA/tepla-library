...
void ec_bn254_fp2_add(EC_POINT R, const EC_POINT P, const EC_POINT Q)
{
	...
	{
		if ( bn254_fp2_cmp(ycoord(P), ycoord(Q)) == 0 ) { ec_bn254_fp2_dob(R, P); return; }
...
void ec_bn254_fp2_dob(EC_POINT R, const EC_POINT P)
{
	...
	if ( bn254_fp2_is_zero(ycoord(P)) ){ point_set_infinity(R); return; }
...
void ec_bn254_fp2_add_formul(EC_POINT R, const EC_POINT P, const EC_POINT Q)
{
	...
	{
		if ( bn254_fp2_is_zero(t[1]) ) { ec_bn254_fp2_dob_formul(R, P); return; }
		point_set_infinity(R); return;
	}
	...
	bn254_fp2_sub(xcoord(R), xcoord(R), t[0]); // Rx = F^2-H-2*I
...
void ec_bn254_fp2_mul_naf(EC_POINT Q, const mpz_t s, const EC_POINT P)
{
	...
	for (i=nlen-2;i>=0;i--)
	{
		...
		if( naf[i] )
		{
			if( naf[i] < 0 ){ ec_bn254_fp2_add_formul(R, R, mP); }
			else { ec_bn254_fp2_add_formul(R, R, P); }
		}
	}
...
void ec_bn254_tw_frob2(EC_POINT Q, const EC_POINT P)
{
	...
	//bn254_fp2_mul_p(ycoord(Q), ycoord(P), vfroby2);
	bn254_fp2_neg(ycoord(Q), ycoord(P));
	element_set(zcoord(Q), zcoord(P));
...
void ec_bn254_fp2_mul_end(EC_POINT Q, const mpz_t s, const EC_POINT P)
{
	...
	mpz_init(s0); mpz_init(s1); mpz_init(s2); mpz_init(s3);
	...
	// create table R:
	//--------------------------------------------
	for(i=0;i<16;i++){ point_init(R[i], curve(P)); }
	...
	for (i=t-1; i>=0; i--)
	{
		...
		if( index ) { ec_bn254_fp2_add_formul(Q, Q, R[index]); }
	}
	...
	//  release
	//--------------------------------------------
	for(i=0;i<16;i++){ point_clear(R[i]); }

	mpz_clear(s0); mpz_clear(s1); mpz_clear(s2); mpz_clear(s3);
}
...
{
	if( point_is_infinity(Q) ){ point_set_infinity(P); return; }
...
int ec_bn254_fp2_cmp(const EC_POINT P, const EC_POINT Q)
{
	if ( bn254_fp2_cmp(xcoord(P), xcoord(Q)) == 0 )
	{
		if( bn254_fp2_cmp(ycoord(P), ycoord(Q)) == 0 ) { return 0; }
	}
	return 1;
}
